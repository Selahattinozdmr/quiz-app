...
export const register = async (prevState: any, formData: any) => {
  ...
  if (isAdmin == "on") {
    ...
    try {
      ...
      const salt = await genSalt(10);
      const hashedPassword = await hash(password, salt);
      const sinifId = await prisma.class.findUnique({
        where: { class: sinif },
      });
...
export const getCheckedSubCategory = async (formData: any) => {
  ...
  if (
    (!math && !relegion && !science && !english && !turkish && !history) ||
    (!mati && !feni && !ingilizcei && !dini && !inkilapi && !turkcei)
  ) {
  ...
  if (math) {
    ...
    math1.map((item: any) => arr.push(item));
  }
  if (relegion) {
    const relegion1 = relegion.split(",");
    relegion1.map((item: any) => arr.push(item));
  }
  if (science) {
    const science1 = science.split(",");
    science1.map((item: any) => arr.push(item));
  }
  if (english) {
    const english1 = english.split(",");
    english1.map((item: any) => arr.push(item));
  }
  if (turkish) {
    const turkish1 = turkish.split(",");
    turkish1.map((item: any) => arr.push(item));
  }
  if (history) {
    const history1 = history.split(",");
    history1.map((item: any) => arr.push(item));
  }
  try {
    const subCategory = await prisma.subCategory.findMany({
      where: { title: { in: [...arr] } },
    });
    const subCategory1 = subCategory.map((item: any) => arr1.push(item.id));
    const question = await prisma.question.findMany({
      where: { subCategoryId: { in: [...arr1] } },
    });
    question.map((item) => {
    ...
    result.map((item: any) => question1.push(item.id));
...
export const approveQuiz = async (formData) => {
  ...
  try {
    const classId = await prisma.class.findUnique({
      where: { class: className1 },
    });

    await prisma.quiz.update({
      where: { id: parseInt(id) },
      data: { isApproved: true, dedline: new Date(date).toISOString() },
    });
    const users = await prisma.user.findMany({
    ...
    users.map(
      async (user) =>
        await prisma.quizUser.create({
          data: { userId: user.id, quizId: parseInt(id) },
        })
    );
  } catch (err) {
    console.log(err);
...
export const getQuizforStudent = async (session) => {
  "use server";
  try {
    const a = await prisma.quizUser.findMany({
...
export const getQuizesforStudend1 = async (id) => {
  ...
  try {
    ...
    const b = await prisma.question.findMany({
      where: { id: { in: [...a[0].quiz.questionId] } },
    });
    return b;
  } catch (err) {
    console.log(err);
...
export const editScore = async (formData) => {
  ...
  try {
    const a = await prisma.quizUser.findUnique({
      where: { id: parseInt(id) },
      include: { quiz: true },
    });
    const sumQ = a?.quiz.sumQuestion;
    const falseA = parseInt(sumQ) - parseInt(score);
    await prisma.quizUser.update({
      where: { id: parseInt(id) },
      data: { trueAnswer: parseInt(score), falseAnswer: falseA },
    });
    console.log(score);
  } catch (err) {
    console.log(err);
